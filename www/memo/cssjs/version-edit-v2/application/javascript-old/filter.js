...
function filterListUpdate(list) {
  ...
  $(list).find('.filter-counter[data-dvz-selector-attribute="any"] .filter-selected-count').html(inCountAll);
}
...
function filterCounterElement(filterElement) {
  var selector = $(filterElement).attr('data-dvz-selector-attribute');
  var parent = findMyListParent(filterElement);
  return $(parent).find('.filter-counter[data-dvz-selector-attribute="' + selector + '"]').first();
}